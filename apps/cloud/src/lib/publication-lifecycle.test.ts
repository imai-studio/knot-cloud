...
describe("publication lifecycle migration", () => {
  ...
  it("prepares an idempotent upload and verifies it for one site", async () => {
    ...
    const retry = await database.query<{ duplicate: boolean }>(
      `SELECT duplicate FROM prepare_asset_upload(
        $1, $2, $3, $4, $5, $6, $7, 'image/png', $8, 'asset.png',
        'asset-upload-key-0001', now() + interval '9 minutes'
      )`,
      [
        tenantA,
        connectorA,
    ...
    expect(retry.rows).toEqual([{ duplicate: true }]);

    await expect(
      database.query(
        "SELECT * FROM commit_asset_upload($1, $2, $3, $4, $5, $6)",
        [tenantA, connectorA, uploadA, assetA, "f".repeat(64), 1],
      ),
    ).rejects.toThrow(/does not match/u);

    const committed = await database.query<{ asset_id: string }>(
      "SELECT asset_id FROM commit_asset_upload($1, $2, $3, $4, $5, $6)",
      [
        tenantA,
        connectorA,
        uploadA,
  ...
  function prepareVersion(
    ...
    return database.query<{
      ...
      `SELECT * FROM prepare_publication_version(
        $1, $2, $3, $4, $5, 'page', $6, '1.0', $7, $8, $9::jsonb, $10
      )`,
      [
        tenantA,
        connectorA,
        ...
        operation,
        contentDigest,
        bundlePath(tenantA, publicationA, versionId, contentDigest),
        JSON.stringify(content),
        `publication-key-${versionId.slice(-4)}`,
      ],
    );
  }
