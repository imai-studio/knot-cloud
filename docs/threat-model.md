...
| Public-page XSS reaches dashboard                   | Closed typed renderer, script-free CSP, and a separate registrable content domain                                                                                                  | P1 browser test                                                                 |
...
| Redis or Queue outage corrupts state                | Postgres remains authoritative; Redis holds only abuse counters and Queue may only wake durable work                                                                               | Durable nonce and command-ledger tests                                          |
...
| Volumetric request or connection exhaustion         | Cloudflare/Vercel edge limits and WAF rules reject abusive traffic before the function; route byte caps and per-connector limits protect only requests that reach application code | Edge configuration and production load/abuse canary before signed-route release |
...
control. Signed connector routes must not be marked released until those controls are configured
and exercised. Application limits remain defense in depth; they are not a substitute for the edge.
